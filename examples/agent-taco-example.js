...
async function agentWithTacoExample() {
  //TODO:  It should be deleted once https://github.com/nucypher/taco-web/pull/701 is merged and the new version is published.
  const { conditions } = await importTaco();
  ...
  // 8. Create encrypted files with balance condition
  console.log('📝 Creating encrypted file with balance condition...');

  // File 1: Balance-gated content (most reliable for testing)
  const nftGatedFile = await agent.create(
    'This content requires minimum ETH balance on Polygon Amoy',
    {
  ...
  try {
    // Decrypt balance-gated content (TACo provider handles auth automatically)
    console.log('🔓 Decrypting balance-gated file...');
    const decryptedNftGatedFile = await agent.getFileContent(
      nftGatedFile.fileId,
      conditionContext
    );
    console.log(
      '✅ Decrypted balance-gated content:',
  ...
  const timeCondition = new conditions.base.time.TimeCondition({
    ...
    returnValueTest: {
      comparator: '>=',
      value: Math.floor(Date.now() / 1000) + 3600, // Accessible till 1 hour from now
    },
  });
  ...
  try {
    ...
    const fileInfo = await agent.getFile(expireInFutureFile.fileId);
  ...
  console.log('\n📊 Summary:');
  console.log('- Created 1 encrypted file with balance access condition');
  console.log('- Demonstrated decryption with automatic authentication');
