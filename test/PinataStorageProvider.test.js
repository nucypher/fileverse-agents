...
describe('PinataStorageProvider Integration Tests', function () {
  ...
  before(function () {
    ...
    if (!process.env.PINATA_JWT || !process.env.PINATA_GATEWAY) {
      throw new Error('Missing required environment variables: PINATA_JWT and PINATA_GATEWAY must be set in .env file');
    }
  ...
  describe('Authentication', function () {
    it('should test Pinata authentication', async function () {
      ...
      if (!authResult || authResult === false) {
        console.log('❌ Authentication failed - but continuing with upload test...\n');
      } else {
  ...
  describe('Upload and Download', function () {
    it('should upload a test file successfully', async function () {
      ...
      expect(uploadResult).to.match(/^ipfs:\/\/(Qm[A-Za-z0-9]{44}|baf[A-Za-z0-9]+)$/);
    ...
    it('should verify upload by downloading the file', async function () {
      ...
      console.log(`✅ Download verified: ${downloaded.data.substring(0, 50)}...\n`);
    });
  });
      ...
      try {
        ...
        expect(error.message).to.satisfy(
          (msg) =>
            ...
            msg.includes('Failed to find file') ||
  ...
  describe('Complete Workflow', function () {
    it('should execute complete upload to unpin workflow', async function () {
      ...
      const workflowUploadResult = await provider.upload(workflowFileName, workflowContent);
